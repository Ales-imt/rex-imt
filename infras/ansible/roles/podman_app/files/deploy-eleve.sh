...
# ── Configuration ────────────────────────────────────────────────────────────
GHCR_IMAGE="ghcr.io/ales-imt/rex-eleve:latest"
CONTAINER_NAME="rex-eleve"
SECRETS_FILE="$(dirname "$0")/secrets.env"
CONFIG_FILE="$(dirname "$0")/config-eleve.yaml"
PORT="8131"
...
docker run -d \
    ...
    --env HOST="$HOST_IP" \
    -v "$(realpath "$CONFIG_FILE")":/opt/rex-eleve/conf/config.yaml:ro,z \
    "$GHCR_IMAGE"

echo "✅ Container élève disponible sur http://localhost:$PORT"
