...
// TestPresenceExposeJustifie dans back-rex-admin/pkg/presence. Toute évolution
...
func fixtureExcuse(t *testing.T, db *pgxpool.Pool) (int64, int32) {
	...
	creerUser := func(role string) int32 {
		...
		err := db.QueryRow(ctx,
			...
			fmt.Sprintf("test-parite-%s-%d@example.invalid", role, suffixe)).Scan(&id)
		if err != nil {
...
func TestPointageExposeJustifie(t *testing.T) {
	db := openDB(t)
	...
	// 2. Contrat HTTP du service étudiant. Le middleware d'authentification est
	// court-circuité : c'est le mapping de la réponse qui est testé, pas lui.
	req := httptest.NewRequest(http.MethodGet,
		fmt.Sprintf("/pointage/seance/%d/presence", seanceID), nil)
	role := auth.RoleGestionnaire
	ctx := context.WithValue(req.Context(), services.PgCtxKey2, &services.Postgres{Db: db})
	ctx = context.WithValue(ctx, auth.SecurityCtxKey, &auth.SecurityCtx{UserID: int(eleveID), Role: &role})
	routeCtx := chi.NewRouteContext()
	routeCtx.URLParams.Add("seanceId", fmt.Sprintf("%d", seanceID))
	ctx = context.WithValue(ctx, chi.RouteCtxKey, routeCtx)

	rec := httptest.NewRecorder()
	GetSeancePresenceHandler(rec, req.WithContext(ctx))
